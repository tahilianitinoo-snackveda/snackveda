...
function serializeBlogPost(p: typeof blogPostsTable.$inferSelect, withContent = true) {
  return {
    ...
    coverImageUrl: p.coverImageUrl, author: p.author, category: p.category,
    tags: (p.tags || "").split(",").map(t => t.trim()).filter(Boolean),
    metaTitle: p.metaTitle, metaDescription: p.metaDescription,
    status: p.status, readMinutes: p.readMinutes,
    publishedAt: p.publishedAt?.toISOString() ?? null,
    createdAt: p.createdAt.toISOString(), updatedAt: p.updatedAt.toISOString(),
  };
}
function slugify(value: string) {
  return value.toLowerCase().trim().replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").slice(0, 90);
}
...
export const handler: Handler = async (event: HandlerEvent, _context: HandlerContext) => {
  ...
  try {
    ...
    if (path === "/blog" && method === "GET") {
      const db = getDb();
      let rows = await db.select().from(blogPostsTable).where(eq(blogPostsTable.status, "published")).orderBy(desc(blogPostsTable.publishedAt));
      if (params.category) rows = rows.filter(r => r.category?.toLowerCase() === String(params.category).toLowerCase());
      if (params.tag) {
        const tag = String(params.tag).toLowerCase();
        rows = rows.filter(r => (r.tags || "").toLowerCase().split(",").map(t => t.trim()).includes(tag));
      }
      if (params.limit) rows = rows.slice(0, Math.max(1, Math.min(50, Number(params.limit))));
    ...
    if (blogSlugMatch && method === "GET") {
      const db = getDb();
      const [post] = await db.select().from(blogPostsTable).where(and(eq(blogPostsTable.slug, blogSlugMatch[1]), eq(blogPostsTable.status, "published"))).limit(1);
      if (!post) return err("Post not found", "NOT_FOUND", 404);
      const related = await db.select().from(blogPostsTable).where(eq(blogPostsTable.status, "published")).orderBy(desc(blogPostsTable.publishedAt)).limit(4);
      return ok({ post: serializeBlogPost(post), related: related.filter(r => r.id !== post.id).slice(0, 3).map(r => serializeBlogPost(r, false)) });
    }
    ...
    if ((path === "/sitemap.xml" || path === "/sitemap") && method === "GET") {
      ...
      try {
        ...
        for (const p of products) entries.push({ loc: `${site}/shop/${p.slug}`, lastmod: p.updatedAt.toISOString().slice(0, 10), priority: "0.7", changefreq: "weekly" });
        const posts = await db.select({ slug: blogPostsTable.slug, updatedAt: blogPostsTable.updatedAt }).from(blogPostsTable).where(eq(blogPostsTable.status, "published"));
        for (const p of posts) entries.push({ loc: `${site}/blog/${p.slug}`, lastmod: p.updatedAt.toISOString().slice(0, 10), priority: "0.7", changefreq: "monthly" });
      } catch (e: any) { console.error("sitemap error:", e?.message); }
      const xml = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${entries
        .map(e => `  <url>\n    <loc>${xmlEscape(e.loc)}</loc>\n    <lastmod>${e.lastmod}</lastmod>\n    <changefreq>${e.changefreq}</changefreq>\n    <priority>${e.priority}</priority>\n  </url>`)
        .join("\n")}\n</urlset>`;
      return { statusCode: 200, headers: { "Content-Type": "application/xml; charset=utf-8", "Access-Control-Allow-Origin": "*", "Cache-Control": "public, max-age=0, s-maxage=3600" }, body: xml };
    }
    ...
    if (path.startsWith("/admin/")) {
      ...
      if (path === "/admin/blog" && method === "POST") {
        ...
        const [row] = await db.insert(blogPostsTable).values({
          ...
          status: d.status || "draft", readMinutes: d.readMinutes || estimateReadMinutes(d.content),
