...
export default function Register() {
  ...
  const onSubmit = (values: z.infer<typeof registerSchema>) => {
    ...
    registerMutation.mutate({ data: payload }, {
      onSuccess: (data: any) => {
        if (data?.token) {
          localStorage.setItem("narayani_token", data.token);
        }
        toast.success(values.accountType === "b2b" ? "Wholesale account created! Welcome to Narayani Distributors." : "Registration successful! Welcome.");
      ...
      <div className="flex-1 flex flex-col items-center justify-center py-16 px-4 bg-muted/20">
        <div className="w-full max-w-2xl bg-card border rounded-3xl p-8 md:p-12 shadow-sm">
          <div className="text-center mb-10">
            ...
            <p className="text-muted-foreground">Join Narayani Distributors for a premium snacking experience</p>
          </div>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              {step === 1 && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                  <h2 className="text-xl font-medium text-center mb-6">How would you like to use Narayani Distributors?</h2>
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    >
                      ...
                      <p className="text-sm text-muted-foreground">Stock Narayani Distributors in your store, cafe, or office.</p>
                    </button>
                  </div>
